...
var getFromBetween = {
  results: [],
  string: "",
  getFromBetween: function getFromBetween(sub1, sub2) {
  ...
  removeFromBetween: function removeFromBetween(sub1, sub2) {
    if (this.string.indexOf(sub1) < 0 || this.string.indexOf(sub2) < 0) return false;
    var removal = sub1 + this.getFromBetween(sub1, sub2) + sub2;
    this.string = this.string.replace(removal, "");
  },
  getAllResults: function getAllResults(sub1, sub2) {
  ...
  get: function get(string, sub1, sub2) {
    this.results = [];
    this.string = string;
    this.getAllResults(sub1, sub2);
    return this.results;
...
function fancyInfo() {
  ...
  $.fancybox.open(testObject, {
    loop: false // buttons: [  // Modify Buttons here - > just add // before line
    // 	"zoom",
    // 	"share",
    // 	"slideShow",
    // 	"fullScreen",
...
function fancyGallery() {
  ...
  imgArray = tmpString.split(','); //console.log(imgArray);
  ...
  for (var _i = 0; _i < finalStringArray.length; _i++) {
    testObject.push({
      src: finalStringArray[_i],
      type: 'image',
      opts: {
        thumb: finalStringArray[_i],
        caption: captionsArray[0][_i]
      }
    });
  }
