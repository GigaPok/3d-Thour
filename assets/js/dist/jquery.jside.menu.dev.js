...
(function ($) {
  ...
  $.fn.jSideMenu = function (options) {
    var setting = $.extend({
      ...
      //possible options position-left or position-right 
      jSideSticky: true,
      // menubar will be fixed on top, false to set static
      jSideSkin: "default-skin",
      // to apply custom skin, just put its name in this string
    ...
    return this.each(function () {
      ...
      target = $(this); //Accessing DOM

      jSide = $(".menu-container, .menu-head"); //hidddenByStart ? $(jSide).addClass('open') : $(jSide).removeClass('open');
      ...
      arrow = $("<i></i>");
      dimBackground = $("<div>"); // Set the height of side menu according to the available height of device

      $(target).css({//'height' : devHeight-headHeight
      });

      if (setting.jSideSticky == true) {//$(".menubar").addClass("sticky");
      ...
      checkActiveMenuItem(); //Dim Layer	

      $(dimBackground).addClass("dim-overlay").appendTo("body"); //Dropdowns

      $(dHeading).click(function () {});
      $('.has-sub.first-element > .dropdown-heading').click(function () {
        $('.has-sub.first-element > .dropdown-heading').not(this).next('ul').css({
          'display': 'none'
        }).removeClass('active');
      ...
      $('.has-sub.sub > .dropdown-heading').click(function () {
        $('.has-sub.sub > .dropdown-heading').not(this).next('ul').css({
          'display': 'none'
        });
      ...
      $(menuTrigger).click(function () {
        ...
        $(jSide).toggleClass("open"); //$(dimBackground).show(setting.jSideTransition);

        $(".menu-body").removeClass("visibility");
        $('.menu-close').addClass("visibility");
      });

      if ($(window).width() > 600 && openByStart == true) {
  ...
  $('li').on('mouseenter', function () {
    ...
    if ($(this).attr('data-target') == undefined || $(this).attr('data-target') == null || $(this).attr('data-target') == 'undefined' || $(this).attr('data-target') == '') {
      ...
      var suffix = '.jpg';
      nodeImg.attr('src', prefix + tmpDataTarget + suffix); // $('.node-img > img').attr('src', url);
  ...
  $('.menu-items > li').on('click tap', function (e) {
    $('.menu-items > li').not(this).removeClass('active').children('ul').css({
      'display': 'none'
    });
